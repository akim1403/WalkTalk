...
public class DirectionActivity extends AppCompatActivity implements OnMapReadyCallback, TaskLoadedCallback {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        getDirection.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                new FetchURL(DirectionActivity.this).execute(getUrl(place1.getPosition(), routePoints[0].getPosition(), "walking"), "walking");
                for (int i = 0; i < routePoints.length - 1; i++) {
                    new FetchURL(DirectionActivity.this).execute(getUrl(routePoints[i].getPosition(), routePoints[i+1].getPosition(), "walking"), "walking");
                }
                new FetchURL(DirectionActivity.this).execute(getUrl(routePoints[routePoints.length-1].getPosition(), place2.getPosition(), "walking"), "walking");
            }
        });
        ...
        goHome.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                new FetchURL(DirectionActivity.this).execute(getUrl(place2.getPosition(), place3.getPosition(), "walking"), "walking");
            }
        });
        ...
        task.addOnSuccessListener(new OnSuccessListener<Location>() {
            ...
            public void onSuccess(Location location) {
                if (location != null) {
                    ...
                    new FetchURL(DirectionActivity.this).execute(getUrl(place3.getPosition(), place1.getPosition(), "walking"), "walking");
    ...
    public double[] getRoutes(LatLng currentLocation) throws Exception {
        ...
        Log.d("myTag", "current = " + lat + "," + lon + "latitude range = " + range[0] + "," + range[1] + "longitude range = "  + range[2] + "," + range[3]);
    ...
    public String[] getWalks(double[] locationMinMax, String walk, Double lat, Double lon) throws Exception {
        ...
        for (int i = 0; i < WalkList.size(); i++) {
            ...
            if (Double.parseDouble(locationDetails[0]) < locationMinMax[0] || Double.parseDouble(locationDetails[0]) > locationMinMax[1]) {
                continue;
            }
            //check if its outside the long smaller than min or bigger than max
            if (Double.parseDouble(locationDetails[1]) < locationMinMax[2] || Double.parseDouble(locationDetails[1]) > locationMinMax[3]) {
                continue;
            }
            ...
            if (walkDetails[1].equals(walkType) && time.equals("Any Length")) {
                String[] infoArr = {walkDetails[0], walkDetails[1], locationDetails[0], locationDetails[1], endLocationDetails[0], endLocationDetails[1], walkDetails[4], walkDetails[5]};
                Log.d("myTag", "returning " + walkDetails[0] + walkDetails[1] + locationDetails[0] + locationDetails[1] + endLocationDetails[0] + endLocationDetails[1] + walkDetails[4]);
                return infoArr;
            }
            //Log.d("myTag", "detials: " + time + walkDetails[4]+ " " + walkType + walkDetails[1]);
