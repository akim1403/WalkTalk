...
public class DirectionReco extends AppCompatActivity implements OnMapReadyCallback, TaskLoadedCallback {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        fusedLocationProviderClient = LocationServices.getFusedLocationProviderClient(this); // Initialize fusedLocationProviderClient
        ...
        String start=bundle.get("start").toString();
        String end=bundle.get("end").toString();
        String[] parts = start.split(":");
        String[] parts2 = end.split(":");
        startLat = Double.parseDouble(parts[0]);
        startLong = Double.parseDouble(parts[1]);
        endLat = Double.parseDouble(parts2[0]);
        endLong = Double.parseDouble(parts2[1]);
        Log.d("myTag", "start = " + startLat + " " + startLong);
        Log.d("myTag", "end = " + endLat + " " + endLong);
        ...
        getLastLocation();
        //place1 = new MarkerOptions().position(new LatLng(38.88404333279727, 121.54452203505474)).title("Location 1");
        //place2 = new MarkerOptions().position(new LatLng(38.86860801450304, 121.53456197968475)).title("Location 2");
        //textLatLong.setText("Latitude: " + place3.getPosition().latitude + ", Longitude: " + place3.getPosition().longitude);
        getDirection.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                //new FetchURL(DirectionActivity.this).execute(getUrl(place1.getPosition(), place2.getPosition(), "walking"), "walking");
                for (int i = 0; i < markerPoints.size(); i++) {
                    LatLng markerPos = new LatLng(Double.parseDouble(markerPoints.get(i).get(0)),  Double.parseDouble(markerPoints.get(i).get(1)));
                ...
                new FetchURL(DirectionReco.this).execute(getUrl(place1.getPosition(), routePoints[0].getPosition(), "walking"), "walking");
                for (int i = 0; i < routePoints.length - 1; i++) {
                    new FetchURL(DirectionReco.this).execute(getUrl(routePoints[i].getPosition(), routePoints[i+1].getPosition(), "walking"), "walking");
                }
                new FetchURL(DirectionReco.this).execute(getUrl(routePoints[routePoints.length-1].getPosition(), place2.getPosition(), "walking"), "walking");
            }
        });
        ...
        goHome.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                new FetchURL(DirectionReco.this).execute(getUrl(place2.getPosition(), place3.getPosition(), "walking"), "walking");
            }
        });
        ...
        homePage.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                Intent intent = new Intent(DirectionReco.this, SecondActivity.class);
        ...
        task.addOnSuccessListener(new OnSuccessListener<Location>() {
            ...
            public void onSuccess(Location location) {
                if (location != null) {
                    ...
                    new FetchURL(DirectionReco.this).execute(getUrl(place3.getPosition(), place1.getPosition(), "walking"), "walking");
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        //this.mMap.addMarker(new MarkerOptions().position(currentLatLng).title("My Current Location"));
    ...
    private String getUrl(LatLng origin, LatLng dest, String directionMode) {
        ...
        String url = "https://maps.googleapis.com/maps/api/directions/" + output + "?" + parameters + "&key=" + "AIzaSyDGkeIui7CMi6Gb_pOZAE7WpaCpLf7EGIo";
        return url;
    }
    ...
    public double[] getRoutes(LatLng currentLocation) throws Exception {
        ...
        Double lon = currentLocation.longitude;
        double[] loc = {startLat,startLong,endLat,endLong};
        //test.setText("current = " + lat + "," + lon + "latitude range = " + range[0] + "," + range[1] + "longitude range = "  + range[2] + "," + range[3]);
        String[] routeInfo = getWalks(loc);
    ...
    public String[] getWalks(double[] locationMinMax) throws Exception {
        ...
        for (int i = 0; i < WalkList.size(); i++) {
            ...
            //Log.d("myTag", startLongLat[1] + " " + endLongLat[0]);

            if (Double.parseDouble(locationDetails[0]) == locationMinMax[0] && Double.parseDouble(locationDetails[1]) == locationMinMax[1]) {
                if (Double.parseDouble(endLocationDetails[0]) == locationMinMax[2] && Double.parseDouble(endLocationDetails[1]) == locationMinMax[3]) {
                    Log.d("myTag", "Found it " + walkDetails[2]);
                    String[] infoArr = {walkDetails[0], walkDetails[1], locationDetails[0], locationDetails[1], endLocationDetails[0], endLocationDetails[1], time, walkDetails[5]};

                    Log.d("myTag", "returning " + walkDetails[0] + walkDetails[1] + locationDetails[0] + locationDetails[1] + endLocationDetails[0] + endLocationDetails[1] + walkDetails[4] + walkDetails[5]);
